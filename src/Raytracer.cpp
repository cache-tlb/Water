...
// r = a*b
// allocate memory for a,b and r before call this function.
...
Vector unProject(int winX, int winY, int winZ, int viewport[4]) {
    ...
    // TODO:  this is actually the PROJ_VIEW_MODEL matrix
...
bool RayTracer::hitTestSphere(const Vector &origin, const Vector &ray, const Vector &center, const float radius, HitTest &result) {
    Vector offset = origin.subtract(center);
    double a = ray.dot(ray);
    double b = 2 * ray.dot(offset);
    double c = offset.dot(offset) - radius * radius;
    double discriminant = b * b - 4 * a * c;
    if (discriminant > 0) {
        double t = (-b - sqrt(discriminant)) / (2 * a);
        Vector delta(ray.x*t, ray.y*t, ray.z*t);
        result.hit = origin.add(delta);
        result.t = t;
        result.normal = result.hit.subtract(center).multiply(1.f/radius);
        return true;
    }
    return false;
}
